...
export const UIEnumValidators = {
  ...
  isValidNotificationType: (value: string): value is NotificationTypes => {
    return Object.values(NotificationTypes).includes(value as NotificationTypes);
  },
...
export const UIEnumHelpers = {
  getLoadingStateOptions: () => Object.values(LoadingStates).map(state => ({ 
    value: state, 
    label: state.charAt(0).toUpperCase() + state.slice(1)
  })),
  
  getThemeOptions: () => Object.values(ThemeOptions).map(theme => ({ 
    value: theme, 
    label: theme.charAt(0).toUpperCase() + theme.slice(1)
  })),
  
  getDeviceTypeOptions: () => Object.values(DeviceTypes).map(device => ({ 
    value: device, 
    label: device.charAt(0).toUpperCase() + device.slice(1)
  })),
  
  getModalTypeOptions: () => Object.values(ModalTypes).map(modal => ({ 
    value: modal, 
    label: modal.charAt(0).toUpperCase() + modal.slice(1)
  })),
  
  getNotificationTypeOptions: () => Object.values(NotificationTypes).map(type => ({ 
    value: type, 
    label: type.charAt(0).toUpperCase() + type.slice(1)
  })),
  
  getButtonVariantOptions: () => Object.values(ButtonVariants).map(variant => ({ 
    value: variant, 
    label: variant.charAt(0).toUpperCase() + variant.slice(1)
  })),
  
  getButtonSizeOptions: () => Object.values(ButtonSizes).map(size => ({ 
    value: size, 
    label: size.charAt(0).toUpperCase() + size.slice(1)
  })),
  
  getInputTypeOptions: () => Object.values(InputTypes).map(type => ({ 
    value: type, 
    label: type.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ')
  })),
  
  getChartTypeOptions: () => Object.values(ChartTypes).map(chart => ({ 
    value: chart, 
    label: chart.charAt(0).toUpperCase() + chart.slice(1)
  })),
  
  getColorSchemeOptions: () => Object.values(ColorSchemes).map(scheme => ({ 
    value: scheme, 
